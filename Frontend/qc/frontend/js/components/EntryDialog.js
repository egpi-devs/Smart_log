...
function renderEntryDialog(title, fields, data = {}, onSave, onClose) {
    ...
    fields.forEach(field => {
        ...
        html += `<label>${field.label} ${requiredAsterisk}</label>`;

        if (field.type === 'checkbox') {
            html += `<div class="checkbox-group">
                <input type="checkbox" id="field-${field.key}" ${val ? 'checked' : ''} ${disabled} ${required}>
                <label for="field-${field.key}">${field.label}</label>
            </div>`;
        } else if (field.type === 'select') {
            html += `<select class="form-input" id="field-${field.key}" ${disabled} ${required}>
                ${(field.options || []).map(o =>
                    `<option value="${o}" ${o === val ? 'selected' : ''}>${o}</option>`
                ).join('')}
            </select>`;
        } else if (field.type === 'textarea') {
            html += `<textarea class="form-input" id="field-${field.key}" ${readOnly} ${required}>${val}</textarea>`;
        } else if (field.type === 'product-search') {
            html += `<input type="text" class="form-input" id="field-${field.key}-search"
                        value="${val}" placeholder="Search products..." oninput="EntryDialog.searchProducts(this.value, '${field.category || ''}')" ${required}>
                     <div class="product-search-list" id="product-search-results"></div>
                     <input type="hidden" id="field-${field.key}" value="${val}" ${required}>`;
        } else {
            ...
            html += `<input type="${inputType}" class="form-input" id="field-${field.key}"
                        value="${val}" placeholder="${field.placeholder || ''}" ${readOnly} ${disabled} ${required}>`;
        }
