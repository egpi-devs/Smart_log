...
def raw_material_detail(request, qc_no):
    ...
    with connection.cursor() as cursor:
        ...
        new_date_str = str(clean_datetime(data['date'] or old_data['date']))
        date_changed = normalize_datetime_str(old_date_str) != normalize_datetime_str(new_date_str)

        if date_changed:
            if not is_superuser:
                return Response({'error': 'Unauthorized: Date modifications are restricted.'}, status=403)
            
            # If superuser: clear audit history and remove edited traces
...
def finished_product_detail(request, qc_no):
    ...
    with connection.cursor() as cursor:
        ...
        if date_changed:
            ...
            # If superuser: clear audit history
            cursor.execute("DELETE FROM AuditLogs WHERE EntityID = %s", [qc_no])
            cursor.execute("""
